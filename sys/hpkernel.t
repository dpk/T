...
(define (call-fault-handler)
  (lap (signal-handler)
    ...
    (equate t-virtual-timer                 (fixnum-ashl 20 2))
...
(define (interrupt_dispatcher)     
  (lap (signal-handler enable-signals gc_interrupt)
    ...
    (equate %%fault-sp-offset 12)
    (equate %%df_d0       32)
    (equate %%df_pc       18)
    (equate fault-quit      3)
    (equate fault-interrupt                   2)
    (equate fault-virtual-timer               20)
                                             
    (move .l (d@r SP 4) S5)                       ; get signal code
    ...
    (jn= %ignore-interrupt)
    (move .l (d@r SP 12) AN)                      ; get context
    (cmp .l ($ fault-virtual-timer) S5)             ; is this a timer interrupt?
    (j= %timer)                                   
    (cmp .l ($ fault-interrupt) S5)                   ; is this a ^q?
    (jn= %fault)                                  ; if so ..
...
%fault
    ...
    (move .b ($ 1) (d@r AN 4))       ; set ok to return bit in context
...
%t-code-interrupt                    
    (move .l AN (@-r SP))                  ; save context
    (move .l (d@r AN %%fault-sp-offset) S0)        ; get fault SP in S0
...
%fault-save-loop                              ; save temps and extra p and s
    ...
    (move .l S1 (@-r SP))
    (move .l nil-reg AN)                                                                   
    (cmp .l (d@r AN slink/kernel-begin) S1)
    (j< %not-in-kernel)
    (cmp .l (d@r AN slink/kernel-end) S1)
    (j> %not-in-kernel)
...
%fault-done                                            
    (asl .l ($ 2) S5)
    (move .l s5 a1)                             ; 1st argument is signal code
